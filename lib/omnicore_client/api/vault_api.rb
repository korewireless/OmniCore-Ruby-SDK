...
module OmniCoreClient
  class VaultApi
    ...
    # @return [Details]
    def enable_vault(subscriptionid, opts = {})
      data, _status_code, _headers = enable_vault_with_http_info(subscriptionid, opts)
      data
    end
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [EnableVault] :enable_vault application/json
    # @return [Array<(Details, Integer, Hash)>] Details data, response status code and response headers
    def enable_vault_with_http_info(subscriptionid, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: VaultApi.enable_vault ...'
      end
      # verify the required parameter 'subscriptionid' is set
      if @api_client.config.client_side_validation && subscriptionid.nil?
        fail ArgumentError, "Missing the required parameter 'subscriptionid' when calling VaultApi.enable_vault"
      end
      # resource path
      local_var_path = '/vault/subscriptions/{subscriptionid}/enable-vault'.sub('{' + 'subscriptionid' + '}', CGI.escape(subscriptionid.to_s))
      ...
      # http body (model)
      post_body = opts[:debug_body] || @api_client.object_to_http_body(opts[:'enable_vault'])

      # return_type
      return_type = opts[:debug_return_type] || 'Details'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['apiKey', 'bearerAuth']

      new_options = opts.merge(
        :operation => :"VaultApi.enable_vault",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
      ...
      data, status_code, headers = @api_client.call_api(:POST, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: VaultApi#enable_vault\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
