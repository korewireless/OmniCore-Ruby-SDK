  ...
  class Device
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OmniCoreClient::Device` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OmniCoreClient::Device`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, name, num_id, parent, registry, blocked, capresent, subscription, created_on, updated_on, credentials, gateway, gateway_config, is_gateway, device_errors, client_online, last_config_ack_time, last_config_send_time, last_error_status, last_error_time, last_event_time, last_heartbeat_time, last_state_time, log_level, metadata, config, state, policy].hash
    end
